...
class ProcessLinkClickJobTest extends TestCase
{
    ...
    public function test_handle_populates_request_context_from_payload(): void
    {
        ...
        $spy = new class(app(\App\Services\Links\ClickVelocityService::class)) extends \App\Services\Links\LinkTrackingService
        {
    ...
    /** A retried job (same dedup_key) must not insert a second click row. */
    public function test_job_with_same_dedup_key_inserts_only_one_click(): void
    {
        $link = $this->makeLink(['clicks' => 0]);
        $payload = [
            'dedup_key' => 'clk_dedup_test_1',
            'ip' => '8.8.8.8',
    ...
    public function test_jobs_with_distinct_dedup_keys_insert_two_clicks(): void
    {
        $link = $this->makeLink();
        $base = [
            'ip' => '8.8.8.8',
        ...
        (new ProcessLinkClickJob($link->id, $base + ['dedup_key' => 'clk_a']))->handle(app(\App\Services\Links\LinkTrackingService::class));
        (new ProcessLinkClickJob($link->id, $base + ['dedup_key' => 'clk_b']))->handle(app(\App\Services\Links\LinkTrackingService::class));

        $this->assertSame(2, Click::where('link_id', $link->id)->count());
    }

    /** Payloads queued before dedup_key existed still dedupe via request_id. */
    public function test_falls_back_to_request_id_when_dedup_key_absent(): void
    {
        $link = $this->makeLink();
        $payload = [
