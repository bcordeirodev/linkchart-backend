...
return [
    ...
    'info' => [
         ...
         */
        'description' => 'Link Chart — URL shortener with advanced analytics. '
            .'JWT-authenticated endpoints expect an `Authorization: Bearer <token>` header; '
            .'public endpoints (shorten, public link/analytics) are open. Success responses use the '
            .'envelope `{ data, meta?, message? }` and errors use `{ error: { code, message, details? } }` '
            .'(applied by the NormalizeApiResponse middleware).',
    ],

    'ui' => [
        'title' => 'Link Chart API',
    ],
     ...
     * ],
     */
    // Document JWT bearer auth based on this project's custom `api.auth` middleware
    // (ApiAuthenticate). Routes behind it (e.g. `api.auth:api`) are marked as
    // requiring the bearer scheme; public routes are documented as open.
    'security_strategy' => [
        \Dedoc\Scramble\SecurityDocumentation\MiddlewareAuthSecurityStrategy::class,
        [
            'middleware' => ['api.auth', 'api.auth:*'],
