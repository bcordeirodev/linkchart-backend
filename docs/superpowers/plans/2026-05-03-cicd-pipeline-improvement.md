...
**Modified:**
- `backend/docker-compose.prod.yml` — Fix Redis healthcheck (add `-a CHANGE_ME_REDIS_PASSWORD --no-auth-warning`, remove `incr ping`)
...
The current `redis-cli --raw incr ping` increments a key named "ping" without supplying the password. With `requirepass CHANGE_ME_REDIS_PASSWORD` set, this returns `NOAUTH Authentication required`. Depending on redis-cli version, exit code is 0 or 1 — unreliable for health checking.
...
```yaml
  redis:
    ...
    networks:
      ...
      - linkcharts-network
    healthcheck:
      test: ["CMD", "redis-cli", "-a", "CHANGE_ME_REDIS_PASSWORD", "--no-auth-warning", "ping"]
      interval: 10s
      timeout: 3s
      retries: 5
      start_period: 30s
...
timeout 60 bash -c "
    until docker exec linkchartredis redis-cli -a CHANGE_ME_REDIS_PASSWORD --no-auth-warning ping 2>/dev/null | grep -q PONG; do
        echo '  Redis starting...'
        sleep 5
    done
"
